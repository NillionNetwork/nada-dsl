...
def test_root_conversion():
    ...
    assert mir.source_files

    operations = mir.operations
    mir_output = mir.outputs[0]
    assert mir_output.name == "output"
    assert mir_output.type == proto_ty.NadaType(
        scalar=proto_ty.ScalarType.SECRET_INTEGER
    )
    assert mir_output.party == "output_party"

    assert operations[mir_output.operation_id].input_ref


def test_input_conversion():
    input = Input(name="input", party=Party("party"))
    input.store_in_ast(proto_ty.NadaType(scalar=proto_ty.ScalarType.SECRET_INTEGER))
    ...
    assert converted.type == proto_ty.NadaType(
        scalar=proto_ty.ScalarType.SECRET_INTEGER
    )
...
def test_array_type_conversion(input_type, type_name, size):
    ...
    assert converted_input.collection.array
...
def test_zip(input_type, input_name):
    ...
    assert zip_mir.type.collection.contained_type.tuple.right == proto_ty.NadaType(
        scalar=proto_ty.ScalarType.SECRET_INTEGER
    )
    assert zip_mir.type.collection.contained_type.tuple.left == proto_ty.NadaType(
        scalar=proto_ty.ScalarType.SECRET_INTEGER
    )
...
def test_unzip(input_type: type[Array]):
    ...
    assert unzip_mir.type.tuple.left == proto_ty.NadaType(
        collection=proto_ty.CollectionType(
            contained_type=proto_ty.NadaType(scalar=proto_ty.ScalarType.SECRET_INTEGER),
            array=proto_ty.Array(size=10),
        )
    )
...
def test_map(input_type, input_name):
    ...
    assert op.type.collection.array
    assert op.type.collection.contained_type == proto_ty.NadaType(
        scalar=proto_ty.ScalarType.SECRET_INTEGER
    )
...
def test_reduce(input_type: type[Array]):
    ...
    assert op.type == proto_ty.NadaType(scalar=proto_ty.ScalarType.SECRET_INTEGER)
...
def test_array_new():
    ...
    assert op.type.collection.array.size == 2
...
def test_tuple_new():
    ...
    assert op.type.tuple.left == proto_ty.NadaType(
        scalar=proto_ty.ScalarType.SECRET_INTEGER
    )
    assert op.type.tuple.right == proto_ty.NadaType(scalar=proto_ty.ScalarType.INTEGER)
...
def test_n_tuple_new():
    ...
    assert op.type.ntuple.fields == [
        proto_ty.NadaType(scalar=proto_ty.ScalarType.SECRET_INTEGER),
        proto_ty.NadaType(scalar=proto_ty.ScalarType.INTEGER),
        proto_ty.NadaType(scalar=proto_ty.ScalarType.SECRET_INTEGER),
    ]
...
def test_object_new():
    ...
    assert op.type.object.fields == {
        "a": proto_ty.NadaType(scalar=proto_ty.ScalarType.SECRET_INTEGER),
        "b": proto_ty.NadaType(scalar=proto_ty.ScalarType.INTEGER),
        "c": proto_ty.NadaType(scalar=proto_ty.ScalarType.SECRET_INTEGER),
    }
...
@pytest.mark.parametrize(
    ("binary_operator", "ty"),
    [
        (operator.add, proto_ty.NadaType(scalar=proto_ty.ScalarType.INTEGER)),
        (operator.sub, proto_ty.NadaType(scalar=proto_ty.ScalarType.INTEGER)),
        (operator.mul, proto_ty.NadaType(scalar=proto_ty.ScalarType.INTEGER)),
        (operator.truediv, proto_ty.NadaType(scalar=proto_ty.ScalarType.INTEGER)),
        (operator.mod, proto_ty.NadaType(scalar=proto_ty.ScalarType.INTEGER)),
        (operator.pow, proto_ty.NadaType(scalar=proto_ty.ScalarType.INTEGER)),
        (operator.lt, proto_ty.NadaType(scalar=proto_ty.ScalarType.BOOLEAN)),
        (operator.gt, proto_ty.NadaType(scalar=proto_ty.ScalarType.BOOLEAN)),
        (operator.le, proto_ty.NadaType(scalar=proto_ty.ScalarType.BOOLEAN)),
        (operator.ge, proto_ty.NadaType(scalar=proto_ty.ScalarType.BOOLEAN)),
        (operator.eq, proto_ty.NadaType(scalar=proto_ty.ScalarType.BOOLEAN)),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            proto_ty.NadaType(scalar=proto_ty.ScalarType.BOOLEAN),
        ),
        (
            operator.gt,
