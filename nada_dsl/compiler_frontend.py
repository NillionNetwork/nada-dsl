...
def print_operations(operation: Dict[int, proto_op.Operation]):
    print()
    for op_id, op in operation.items():
...
def type_to_str(ty: proto_ty.NadaType):
    if hasattr(ty, "scalar"):
        return str(ty.scalar).lower()
    if hasattr(ty, "collection"):
        if hasattr(ty.collection, "array"):
